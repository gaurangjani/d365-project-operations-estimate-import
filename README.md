...
- **Delete works, but only for lines created after this fix.** Dataverse's Delete trigger for this connector doesn't expose the deleted row's other field values, so the flow can't read its stored Cost/Sales line lookups at delete time. The fix: every Cost Estimate Line gets stamped with a back-reference (`est_sourceimportid`, a custom field added to the standard `msdyn_estimateline` table) at creation time, and the Delete branch looks the line up by that stamp instead. Verified end-to-end — deleting a staging row really does delete its paired lines now. The one gap: a Cost line created *before* this fix has no stamp, so deleting its staging row is a silent no-op for it (nothing to clean up, nothing breaks) — it self-heals the first time that row is edited, since Update also stamps the reference. If you need older rows fixed immediately rather than waiting for an edit, back-fill the stamp manually from each staging row's Cost Estimate Line lookup.
...
2. **Reconnect the flow's Dataverse connection**
   Power Automate connection references are environment-specific. After import, open **Community - Process Estimate Import** in Power Automate, reconnect the Dataverse connection reference, and turn the flow **On** if it isn't already.
